...
class LSTMModel(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int, hidden_dim: int, num_layers: int, dropout: float, tie_weights: bool = True):
        ...
        self.rnn = LSTM(self.vocab_size, self.embedding_dim, self.hidden_dim, self.num_layers, dropout=self.dropout, batch_first=True)
    ...
    def forward(self, input_ids: torch.Tensor, hidden: torch.Tensor = None, output_hidden_states: bool = False):
    ...
    def init_weights(self):
        ...
        for i in range(self.num_layers):
            for name, param in self.rnn.named_parameters():
                if 'weight' in name:
                    param.data.uniform_(-initrange_other, initrange_other)
                elif 'bias' in name:
                    param.data.zero_()
